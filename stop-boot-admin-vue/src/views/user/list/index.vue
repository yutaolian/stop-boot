<template>
    <div class="app-container">
        <!--分页过滤条件-->
        <div v-permission="['P_USER_LIST_PAGE']" class="filter-container" >
            <el-form ref="filterForm" :model="tableQuery">
                <el-row>
                    ...
                    <el-col :span="4">
                        <el-form-item label="">
                            <el-button v-permission="['P_USER_LIST_PAGE']" class="filter-item" type="danger" icon="el-icon-close" @click="cleanFilter" circle/>
                            <el-button v-permission="['P_USER_LIST_PAGE']" class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter" circle/>
                            <el-button v-permission="['P_USER_LIST_ADD']" class="filter-item" type="success" icon="el-icon-plus" @click="preCreate" circle/>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
        ...
        >
            ...
            <el-table-column label="Actions" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{row}">
                    <el-button v-permission="['P_USER_LIST_ONE']"  type="primary" size="mini" @click="preEdit(row)">
                        Edit
                    </el-button>
                    <el-button v-permission="['P_USER_LIST_DLETET']" size="mini" type="danger" @click="handleDelete(row)">
                        Delete
                    </el-button>
                </template>
            </el-table-column>
        ...
        <!--分页组件-->
        <pagination  v-permission="['P_USER_LIST_PAGE']" v-show="total>0" :total="total" :page.sync="tableQuery.pageNum" :limit.sync="tableQuery.pageSize"
                    @pagination="loadData"/>

        <!--新增组件-->
        <create-form v-permission="['P_USER_LIST_ADD']"  ref="createForm" :rowData='createRowData'  @loadData="loadData"></create-form>

        <!--编辑组件-->
        <edit-form v-permission="['P_USER_LIST_UPDATE']" ref="editForm" :rowData='editRowData'  @loadData="loadData"></edit-form>
...
<script>
    ...
    //UserList delete 接口
    import {UserListDeleteRequest} from '@/sdk/api/user/list/delete'
        ...
        methods: {
            loadData() {
                this.$nextTick(() => {
                    ...
                    request.api().then(res => {
                        ...
                        console.log("UserList tableData res:", res)
                    })
                })
            },
            handleFilter() {
            ...
            handleDelete(row) {
                this.$confirm('确认删除, 是否继续?', '提示', {
                    ...
                    let request = new UserListDeleteRequest();
