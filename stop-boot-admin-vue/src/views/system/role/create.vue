  ...
  <el-dialog title="新增" :visible.sync="dialogFormVisible" width="30%">
    <el-form ref="createFormRef" :rules="rules" :model="createFormData" label-position="left" label-width="100px">
      ...
      <el-form-item label="description" prop="description">
        <el-input v-model="createFormData.description"/>
      </el-form-item>
      <el-form-item label="tag" prop="tag">
        <el-input v-model="createFormData.tag"/>
      </el-form-item>
...
<script>
    //接口
    import {RoleAddRequest} from '@/sdk/api/system/role/add'
                ...
                this.$refs['createFormRef'].validate((valid) => {
                    if (valid) {
                        this.$confirm('此操作将提交数据, 是否继续?', '提示', {
                            ...
                            let request = new RoleAddRequest();
                            request.setParams(this.createFormData)
                                ...
                                console.log("RoleAddRequest res:", res)
                            })
                        }).catch((err) => {
                            this.$message({
                                type: 'info',
